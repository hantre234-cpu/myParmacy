...
$pageTitle  = 'Modifier médicament';
$activePage = 'medicine';

$pdo = db();
$id  = (int)($_GET['id'] ?? 0);

$med = $pdo->prepare("SELECT * FROM medicines WHERE id = ?");
$med->execute([$id]);
$med = $med->fetch();

if (!$med) { flash('error','Médicament introuvable.'); redirect(BASE_URL.'/medicine/index.php'); }
    ...
    if (empty($errors)) {
        $pdo->prepare("
            UPDATE medicines SET
                ...
                price=?, stock=?, expiry_date=?, description=?, requires_rx=?
            WHERE id=?
        ")->execute([
            $data['name'], $data['generic_name'], $data['category_id'], $data['dosage'],
            $data['price'], $data['stock'], $data['expiry_date'], $data['description'],
            $data['requires_rx'], $id
        ]);
...
<div class="container-xl section-pad">
  <div class="row justify-content-center">
    <div class="col-lg-8">
      ...
      <h1 class="section-title mb-1">Modifier le médicament</h1>
      <p class="section-sub mb-4"><?= h($med['name']) ?></p>
        ...
        <form method="post" novalidate>
          <div class="row g-4">
            ...
            <div class="col-md-5">
              ...
              <input type="text" name="generic_name" value="<?= h($data['generic_name']) ?>" class="pc-input form-control">
            </div>
            ...
            <div class="col-md-6">
              ...
              <input type="text" name="dosage" value="<?= h($data['dosage']) ?>" class="pc-input form-control">
            </div>

            <div class="col-md-4">
              <label class="form-label">Prix (DA) *</label>
              <input type="number" name="price" value="<?= h($data['price']) ?>" min="0" step="0.01"
            ...
            <div class="col-md-4">
              ...
              <input type="number" name="stock" value="<?= h($data['stock']) ?>" min="0" class="pc-input form-control">
            </div>

            <div class="col-md-4">
              <label class="form-label">Date d'expiration</label>
              <input type="date" name="expiry_date" value="<?= h($data['expiry_date'] ?? '') ?>" class="pc-input form-control">
            </div>

            <div class="col-12">
              <label class="form-label">Description</label>
              <textarea name="description" rows="3" class="pc-input form-control"><?= h($data['description']) ?></textarea>
            </div>
            ...
            <div class="col-12">
              <div class="form-check">
                ...
                <label class="form-check-label" for="requires_rx" style="color:var(--clr-muted);font-size:.88rem">
                  <span class="badge-amber me-1">Rx</span> Requiert une ordonnance
                </label>
              </div>
            </div>
          </div>
